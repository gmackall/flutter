...
sk_sp<DlRuntimeEffect> DlRuntimeEffectImpeller::Make(
  ...
  if (runtime_stage) {
    ...
    for (const auto& u : runtime_stage->GetUniforms()) {
      FML_LOG(ERROR) << "  Name: " << u.name << " Size: " << u.GetSize()
                     << " Type: " << (int)u.type;
    }
  }
  if (skia_effect) {
    ...
    for (const auto& u : skia_effect->uniforms()) {
      FML_LOG(ERROR) << "  Name: " << u.name << " Offset: " << u.offset
                     << " Size: " << u.sizeInBytes();
    }
  }
...
DlRuntimeEffectImpeller::GetSkiaUniformData(
  ...
  auto skia_data =
      std::make_shared<std::vector<uint8_t>>(skia_effect_->uniformSize());
  ...
  const std::string kReservedBlockName =
      "_RESERVED_IDENTIFIER_FIXUP_gl_DefaultUniformBlock";
  bool has_reserved_block =
      impeller_uniforms.find(kReservedBlockName) != impeller_uniforms.end();
  ...
  if (has_reserved_block) {
    ...
    FML_LOG(ERROR) << "HI GRAY, Found reserved block at offset: "
                   << reserved_block_offset;
  }
  ...
  for (const auto& sk_uniform : skia_effect_->uniforms()) {
    ...
    // Special handling for u_texture_size which might be missing from Impeller
    // reflection but is required by Skia (and should match u_size).
    if (sk_name == "u_texture_size") {
      FML_LOG(ERROR) << "HI GRAY, Injecting u_texture_size from u_size";
      // We assume u_size is at offset 0 of the data if we are using the
      // reserved block, OR we can look up u_size in the map? But u_layout
      // generally matches the reserved block layout for these fields. Let's
      // assume u_size is the first 8 bytes of the data if the reserved block is
      // at 0. The reserved block is usually the first thing for these shaders.
      // Let's confirm if we found a reserved block.
      auto block_it = impeller_uniforms.find(
          "_RESERVED_IDENTIFIER_FIXUP_gl_DefaultUniformBlock");
      if (block_it != impeller_uniforms.end()) {
        const auto& block_uniform = block_it->second;
        // u_size should be at the start of this block.
        // We need to calculate the actual offset of the reserved block in the
        // impeller_data.
        ...
        if (block_uniform.GetSize() >= 8) {
          const uint8_t* val =
              impeller_data->data() +
              current_reserved_block_offset;  // u_size is at start of block
          uint8_t* dst = skia_data->data() + sk_uniform.offset;
          memcpy(dst, val, 8);  // Copy 8 bytes (vec2)
          FML_LOG(ERROR) << "HI GRAY, Injected u_texture_size: " << *(float*)val
                         << ", " << *(float*)(val + 4);
        }
      }
      continue;
    ...
    if (it == impeller_uniforms.end() && has_reserved_block) {
      ...
      // We read from impeller_data at (reserved_block_offset +
      // sk_uniform.offset)
      ...
      if (source_offset + copy_size <= impeller_data->size() &&
        ...
        memcpy(skia_data->data() + sk_uniform.offset,
               impeller_data->data() + source_offset, copy_size);
      } else {
    ...
    if (it != impeller_uniforms.end()) {
      ...
      // We need to calculate the offset in the *packed data buffer*
      // (impeller_data). FragmentProgram packs them in order of GetUniforms().
      ...
      size_t copy_size =
          std::min(sk_uniform.sizeInBytes(), imp_uniform.GetSize());
      ...
      if (packed_offset + copy_size <= impeller_data->size() &&
          sk_uniform.offset + copy_size <= skia_data->size()) {
        memcpy(skia_data->data() + sk_uniform.offset,
               impeller_data->data() + packed_offset, copy_size);
      } else {
        FML_LOG(ERROR) << "HI GRAY, Bounds check failed for "
                       << sk_uniform.name;
      }
    } else {
      FML_LOG(ERROR) << "HI GRAY, Impeller uniform not found for "
                     << sk_uniform.name;
    }
  }
