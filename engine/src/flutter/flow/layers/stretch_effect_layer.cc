...
StretchEffectLayer::StretchEffectLayer(
...
void StretchEffectLayer::Diff(DiffContext* context, const Layer* old_layer) {
  DiffContext::AutoSubtreeRestore subtree(context);
  auto* prev = static_cast<const StretchEffectLayer*>(old_layer);
  if (!context->IsSubtreeDirty()) {
    FML_DCHECK(prev);
    if (NotEquals(filter_, prev->filter_) || offset_ != prev->offset_ ||
        bounds_ != prev->bounds_ || stretch_x_ != prev->stretch_x_ ||
        stretch_y_ != prev->stretch_y_ ||
...
void StretchEffectLayer::Preroll(PrerollContext* context) {
  auto mutator = context->state_stack.save();
  mutator.translate(offset_);
  // Record the stretch before prerolling children so that any platform view
  // in the subtree picks it up when it fills its mutators stack.
  mutator.applyStretchEffect(bounds_, stretch_x_, stretch_y_,
...
void StretchEffectLayer::Paint(PaintContext& context) const {
