...
/// Applies framework-side path clipping to a platform view, under the
/// composition mode selected by `--dart-define=PV_MODE`. Run under TLHC + HCPP.
/// Asserted by `test_driver/core/clippath_test.dart`.
void main() async {
  ensureAndroidDevice();
...
class _ClipperHomePageState extends State<ClipperHomePage> {
  ...
  Widget build(BuildContext context) {
    ...
    final Widget contentToClip = ClipOval(
      // Inner ClipOval remains
      child: Stack(
        alignment: Alignment.center,
        children: <Widget>[
          // Background
          const SizedBox.square(dimension: 500, child: ColoredBox(color: Colors.green)),
          SizedBox.square(
            dimension: 400,
